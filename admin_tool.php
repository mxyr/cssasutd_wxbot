	...
	if ( $deco_msg == null ) { //非json格式：短命令或者普通消息
		...
		switch ( $deco_msg->命令 ) {
			...
			case '群发链接':
				...
				if ( $deco_msg->参数 != null and is_string( $deco_msg->参数->标题 )and is_string( $deco_msg->参数->描述 )and is_string( $deco_msg->参数->链接 )and is_string( $deco_msg->参数->缩略图链接 ) ) {
					$title = $deco_msg->参数->标题;
					$text = $deco_msg->参数->描述;
					$target_url = $deco_msg->参数->链接;
					$pic_url = $deco_msg->参数->缩略图链接;
					foreach ( $bot->config[ 'group_main' ] as $name => $qun_id ) {
						sleep( 1 );
						if ( in_array( $qun_id, $gp_array ) ) {
							$bot->sendLinkMsg( $title, $text, $target_url, $pic_url, $bot->robot_wxid, $qun_id );
						} else {
							$bot->sendTextMsg( $qun_id . '不在群聊列表，群发消息失败！' );
						}
					}
